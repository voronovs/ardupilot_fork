...
gcs:send_text(7, string.format("Serial number - FLUAVFLY202512М%d", brd_serial_number))
gcs:send_text(7, string.format("FW version - FLYCO_4.6.3.3"))
gcs:send_text(7, string.format("Parameters version - 20260205"))
gcs:send_text(7, string.format("Script version - 20260205"))
...
function updateParameters ()
    if state_with_payload and change_params_flag then
        param:set_and_save("ATC_RAT_PIT_D", 0.01020357)
        param:set_and_save("ATC_RAT_PIT_I", 0.17057)
        param:set_and_save("ATC_RAT_PIT_P", 0.17057)
        param:set_and_save("ATC_RAT_RLL_D", 0.009808756)
        param:set_and_save("ATC_RAT_RLL_I", 0.1640365)
        param:set_and_save("ATC_RAT_RLL_P", 0.1640365)
    ...
    elseif state_empty and change_params_flag then
        param:set_and_save("ATC_RAT_PIT_D", 0.007505372)
        param:set_and_save("ATC_RAT_PIT_I", 0.1354057)
        param:set_and_save("ATC_RAT_PIT_P", 0.1354057)
        param:set_and_save("ATC_RAT_RLL_D", 0.007066483)
        param:set_and_save("ATC_RAT_RLL_I", 0.1268779)
        param:set_and_save("ATC_RAT_RLL_P", 0.1268779)
...
function updateBatteryInfo()
    ...
    if current ~= nil and  voltage ~= nil then
        watts = 2*current*voltage -- watts from 6S2P, W
    ...
    if (update_user) then
        ...
        if math.floor(capacity_used) >= math.floor(batt_capacity)*0.85 then
            gcs:send_text(3, string.format("%d / %d mAh, %s A, %s V", math.floor(capacity_used), math.floor(batt_capacity), tostring(current*2), tostring(voltage)))
        elseif math.floor(capacity_used) < math.floor(batt_capacity)*0.85 and math.floor(capacity_used) >= math.floor(batt_capacity)*0.45 then
            gcs:send_text(5, string.format("%d / %d mAh, %s A, %s V", math.floor(capacity_used), math.floor(batt_capacity), tostring(current*2), tostring(voltage)))
        elseif math.floor(capacity_used) < math.floor(batt_capacity)*0.5 then
            gcs:send_text(7, string.format("%d / %d mAh, %s A, %s V", math.floor(capacity_used), math.floor(batt_capacity), tostring(current*2), tostring(voltage)))
        end

        if sample_count > 0 then
            avg_watts = watts_sum / sample_count

            if math.floor(avg_watts) > 1400 and state_empty then
                change_params_flag = true
                state_with_payload = true
                state_empty = false
                updateParameters()
            elseif math.floor(avg_watts) <= 1400 and state_with_payload then
                change_params_flag = true
